...
class Home extends Component {
  ...
  onChangeTab = tabId => {
    this.setState({activePage: tabId})
  }

  checkPicture = pictureId => {
    const {gamePictureId} = this.state
    ...
    if (gamePictureId === pictureId) {
      ...
      this.setState(prevState => ({
        ...
        gamePictureUrl: newArray.imageUrl,
        gamePictureId: newArray.id,
      }))
    } else {
      this.clearInterval()
      this.setState({isGameEnded: true})
  ...
  render() {
    ...
    const thumbNailsList = imagesList.filter(
      eachImage => eachImage.category === activePage,
    )

    return (
      <>
        <NavBar score={score} remainingTime={timeInSec} />
        <div className="home-main-app">
          {isGameEnded ? (
            <ScoreCard score={score} onRefresh={this.refreshGame} />
          ) : (
            <div className="home-container">
              <img src={gamePictureUrl} alt="match" className="top-image" />
              <ul className="list-tabs-container">
              ...
              <ul className="thumbnails-list-container">
                {thumbNailsList.map(eachCard => (
                  <ThumbnailsItem
                    details={eachCard}
                    key={eachCard.id}
                    checkPicture={this.checkPicture}
                  />
                ))}
              </ul>
            </div>
